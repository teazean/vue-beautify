...
<li class="tpl-item clearfix">
  <div class="tpl-status">
    <span class="label bg-info label-status-{{tpl.status}}">{{tpl.status | statusDesc}}</span>
  </div>

  <div class="tpl-type">
    <span>[{{tpl.materialType == 'MOBILE' ? '手机' : 'PC'}}]</span>
  </div>

  <div class="pull-left preview-link">
    <a target="_blank" href="{{tpl.previewUrl}}">{{tpl.previewUrl}}</a>
